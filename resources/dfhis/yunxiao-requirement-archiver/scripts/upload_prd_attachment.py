#!/usr/bin/env python3
"""Upload PRD_AND_CODE_ANALYSIS.md as a Yunxiao work-item attachment."""
...
DEFAULT_PRD_NAME = "PRD_AND_CODE_ANALYSIS.md"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--requirement-dir",
        required=True,
        help="Local requirement directory containing raw.json and PRD_AND_CODE_ANALYSIS.md.",
    )
    ...
    parser.add_argument("--file-name", default=DEFAULT_PRD_NAME, help="Attachment file name.")
...
def main() -> int:
    ...
    prd_path = requirement_dir / DEFAULT_PRD_NAME
    if not prd_path.exists():
        print(f"error: {DEFAULT_PRD_NAME} not found under requirement dir: {requirement_dir}", file=sys.stderr)
        return 2
    ...
    try:
        ...
        matching = [
            ...
            if str(item.get("fileName") or item.get("name") or "") == file_name
        ...
        if matching and not args.force:
            ...
            upload_result = client.call_tool(
                "create_workitem_attachment",
                {
                    ...
                    "fileContent": base64.b64encode(prd_path.read_bytes()).decode("ascii"),
                    "fileName": file_name,
                },
                4,
            )
        ...
        verified = any(
            str(item.get("fileName") or item.get("name") or "") == file_name
            and int(item.get("size") or -1) == file_size
