...
- Any code change: unit-test `passing_test` evidence is mandatory (test file paths, scenario count, run result); zero-test delivery is unqualified and is flagged by later AI scans.
...
10. Verify locally. Prefer `lint`, `build`, or syntax checks from the repo scripts. For frontend repositories, follow the DFHIS frontend verification environment gate above before declaring dependency/tooling blockers. If private dependencies still block verification after the correct Node/package-manager attempt, record the exact blocker in both the chat summary and the handoff document. **Unit tests are mandatory (硬性规则，用户明确要求，不可豁免): every HIS code change must add or extend unit tests covering the changed main scenario plus regression coverage of every touched branch, following the repository's existing test conventions (e.g. `tests/<功能目录>/xxx.test.js` runnable with plain `node` for df-web frontends, the repo's existing JUnit test tree for backends). Run the unit tests and record the result before push. A change delivered without unit tests is unqualified — subsequent AI scans flag it as a non-compliant modification; `build passed` or `lint passed` can never substitute for unit tests. If the repository has no unit test surface yet, create one with the lightest convention already used in HIS repos — missing infrastructure is not an exemption.**
