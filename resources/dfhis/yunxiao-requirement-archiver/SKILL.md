...
7. Clone or update only the target repository on the local machine using local Git credentials. Use `scripts/prepare_local_worktree.py` to run `git clone`, `git fetch`, and `git worktree add`. Put requirement-specific code worktrees under `{需求目录}/code/{repo-name}` so requirement evidence and code stay together. Branch naming is based on Yunxiao work-item type: defects/bugs use `hotfix-DFHIS-12345`; requirements/features use `feature-DFHIS-12345`. If the type is unknown, inspect `raw.json`/`requirement.md` first instead of guessing.
   **例外（不建需求分支，直接改 dev）**：共享 API 仓库 `df-his-api`（以及后续加入 `DIRECT_DEV_REPOS` 的共享仓库）不创建 feature/hotfix 分支，契约改动直接在 dev 分支上提交——df-his-api 的 dev 分支即发布主线，契约改动直接进 dev，由发布流程统一出 API jar。准备 worktree 用：`prepare_local_worktree.py --remote <df-his-api remote> --repo-dir <缓存> --worktree-dir {需求目录}/code/df-his-api --base-ref origin/dev --branch dev --direct-dev`（`--direct-dev` 以 detached HEAD 检出 origin/dev，避免多需求 worktree 争用同名 dev 分支）。提交后 `git push origin HEAD:dev`。`guard_code_edit.py` 已允许该模式（git root 目录名在 `DIRECT_DEV_REPOS` 内时，接受 `dev` 分支或 detached HEAD）。
8. Before every code edit, run `scripts/guard_code_edit.py --requirement-dir {需求目录} {待编辑文件...}` and confirm it prints `ok`. This is mandatory even when the target file path looks obvious. The guard must validate that each edited path is under `{需求目录}/code/<repo>` and that the branch is `feature-DFHIS-12345` or `hotfix-DFHIS-12345` (or, for repos in `DIRECT_DEV_REPOS` such as `df-his-api`, the `dev` branch or a detached HEAD at `origin/dev`); if it fails, fix the worktree setup first and do not edit the original workspace.
...
11. Commit and push the branch from the local machine. If this requirement came from an Orca Yunxiao todo pool claim, the git commit message must be exactly the full Yunxiao URL from the claim's `提交信息` or `链接` field, and nothing else. Do not replace it with only `DFHIS-12345`, the title, a summary, or a conventional commit message. Push explicitly to the requirement branch, for example `git push -u origin feature-DFHIS-12345`, then verify `git status -sb` so the local branch tracks the pushed feature/hotfix branch rather than the RC base. **Direct-dev repos (df-his-api)**: no feature/hotfix branch exists, push straight to dev with `git push origin HEAD:dev` (detached HEAD created by `--direct-dev`). Do not upload patches to `192.168.1.10` for server-side pushing.
...
Shared API repos (df-his-api) work directly on dev without a feature/hotfix branch:

```bash
python3 scripts/prepare_local_worktree.py \
  --remote <df-his-api-remote> \
  --repo-dir /path/to/local/cache/df-his-api \
  --worktree-dir "$YUNXIAO_REQUIREMENT_DIR/code/df-his-api" \
  --base-ref origin/dev \
  --branch dev --direct-dev        # detached HEAD at origin/dev, no new branch
# then inside the worktree:
python3 scripts/guard_code_edit.py --requirement-dir "$YUNXIAO_REQUIREMENT_DIR" \
  "$YUNXIAO_REQUIREMENT_DIR/code/df-his-api/path/to/contract.java"
git add <changed-files>
git commit -m "https://devops.aliyun.com/projex/req/DFHIS-12345"
git push origin HEAD:dev            # push commits straight to dev
```

`guard_code_edit.py` accepts `dev` branch or detached HEAD only when the git root directory name is in `DIRECT_DEV_REPOS` (currently `df-his-api`). All other repositories keep the feature/hotfix branch requirement.
