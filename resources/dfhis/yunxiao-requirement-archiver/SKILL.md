...
- DFHIS build/API guardrail: do not modify build or dependency files such as `build.gradle`, `settings.gradle`, `pom.xml`, or dependency lock files to solve a requirement. Do not switch published dependencies to `compile project(...)`, do not enable local project API modules, and do not add/modify/rely on project-local `*-api` / API modules, DTOs, Req classes, Feign clients, or client API packages; these project APIs are deprecated and invalid for new requirement work. If a solution appears to require any build/API-module change, mark the Requirement Contract as `needs_clarification` or `blocked` and request architecture/product confirmation before code edits.
...
9. Implement the smallest code change that matches the evidence and the handoff document. Do not modify unrelated repositories or formatting. Do not modify `build.gradle`/`settings.gradle`/`pom.xml`/dependency lock files, switch to `compile project(...)`, or touch project-local `*-api` / API modules. If compilation or implementation seems to require those changes, stop and update the contract as `needs_clarification` or `blocked`.
