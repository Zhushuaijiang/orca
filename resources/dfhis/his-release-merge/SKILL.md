...
description: HIS 发版分支间按需求 cherry-pick 合并。用于根据钉钉/Excel 合并清单，把多个 HIS 服务或前端包的指定需求分支合并到 release/RC 分支；必须先按服务分类、复制或克隆到隔离工作区，禁止直接修改原始仓库目录；合并推送成功后必须向钉钉群发送合并通知（见 dingtalk-group-notify）。当不能整体 merge 长生命周期分支、需要跨分支合并提交、发版合并、需求合并到 release、合并后发群通知时使用。
---
...
11. **推送成功后必须发群通知**：按 `dingtalk-group-notify` 向 HIS 团队钉钉群发送合并完成通知（目标分支、仓库、需求号、sha 区间、验证结果、冲突说明）；**通知未发出不算任务完成**，推送失败时禁止发“已完成”通知。
...
### 6.3 推送成功后发送群通知（强制）
推送成功后，按 `dingtalk-group-notify` 技能向钉钉群发送合并完成通知，内容至少含：

- 目标分支名、各仓库/服务、需求号 + sha 区间（`旧..新`）
- 验证结论（编译/单测）
- 冲突处理说明（有冲突时）

校验响应 `errcode == 0` 才算发送成功；**推送成功后才发**，通知未发出不得汇报任务完成。
...
13. **群通知的时机与 @ 人限制**：通知必须在推送成功后发（推送失败发“已完成”属误导）；钉钉机器人 @ 只能用手机号或钉钉 userId，云效 userId 无效，取不到手机号时直接发群不 @ 或先问用户（细节见 `dingtalk-group-notify`）。
...
全部完成后给汇总表 + 冲突处理说明 + 验证命令结果 + 待人工复核项 + 隔离仓库路径。推送成功并经 `dingtalk-group-notify` 发出群通知后，才算交付完成。不要声称“完美合并”，只说明已完成的验证范围和剩余风险。
