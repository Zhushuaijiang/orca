...
function redactText(value) {
  ...
  for (const name of ['DFHIS_USERNAME', 'DFHIS_PASSWORD']) {
    ...
    if (secret) result = result.replaceAll(secret, `<${name.toLowerCase()}-redacted>`)
  }
  return result.slice(0, 1000)
}

function writeJUnit({ duration, error }) {
  const failure = error ? `<failure message="${xml(error.message)}">${xml(error.stack || error.message)}</failure>` : ''
...
async function main() {
  if (process.env.DFHIS_UI_SANDBOX !== '1') throw new Error('Run through run_container_ui_test.py')
  ...
  let failure
  try {
    context = await chromium.launchPersistentContext(fs.mkdtempSync(path.join(os.tmpdir(), 'dfhis-profile-')), {
      headless: true,
    ...
    page.on('response', (response) => responses.push({ status: response.status(), url: safeUrl(response.url()) }))
    page.on('console', (message) => {
      if (['warning', 'error'].includes(message.type())) {
        consoleMessages.push({ type: message.type(), text: redactText(message.text()) })
      }
    })
    page.on('pageerror', (error) => consoleMessages.push({ type: 'pageerror', text: redactText(error.message) }))
    ...
    if (page) await page.screenshot({ path: path.join(screenshotDir, 'scenario_failed.png'), fullPage: true }).catch(() => {})
  } finally {
    fs.writeFileSync(path.join(networkDir, 'responses.json'), `${JSON.stringify(responses, null, 2)}\n`)
    fs.writeFileSync(path.join(networkDir, 'console.json'), `${JSON.stringify(consoleMessages, null, 2)}\n`)
