...
// Fork-only bridges; the contracts stay inline in ./api-types until upstream absorbs them.
const yunxiaoApi: PreloadApi['yunxiao'] = {
  listWorkItems: (args) => ipcRenderer.invoke('yunxiao:listWorkItems', args),
  createRequirement: (args) => ipcRenderer.invoke('yunxiao:createRequirement', args),
  listRequirementFieldOptions: () => ipcRenderer.invoke('yunxiao:listRequirementFieldOptions'),
  archiveRequirement: (args) => ipcRenderer.invoke('yunxiao:archiveRequirement', args),
  listTodoPool: () => ipcRenderer.invoke('yunxiao:listTodoPool'),
  addTodoPoolItems: (args) => ipcRenderer.invoke('yunxiao:addTodoPoolItems', args),
  updateTodoPoolItem: (args) => ipcRenderer.invoke('yunxiao:updateTodoPoolItem', args),
  removeTodoPoolItem: (id) => ipcRenderer.invoke('yunxiao:removeTodoPoolItem', id)
}

const dfhisEnvironmentApi: PreloadApi['dfhisEnvironment'] = {
  getConfig: () => ipcRenderer.invoke('dfhisEnvironment:getConfig'),
  check: () => ipcRenderer.invoke('dfhisEnvironment:check'),
  install: (config) => ipcRenderer.invoke('dfhisEnvironment:install', config)
}

const agentSessionApi: PreloadApi['agentSession'] = {
  findLockHolders: (sessionId) => ipcRenderer.invoke('agentSession:findLockHolders', sessionId),
  killLockHolder: (pid) => ipcRenderer.invoke('agentSession:killLockHolder', pid)
}
...
const api = {
  ...
  shell: shellApi,
  skills: skillsApi,
  dfhisEnvironment: dfhisEnvironmentApi,
  pet: petApi,
  browser: browserApi,
