...
export abstract class UpdaterMenuChecks extends UpdaterScheduling {
  protected checkForUpdatesFromMenu(options?: UpdateCheckOptions): void {
    ...
    if (this.isReleaseFeedDisabled()) {
      // Why: DFHIS builds ship with the GitHub feed disabled; check the DFHIS download server's latest.json instead of reporting "no updates".
      this.closeLocalBuildFeed()
      this.activeUpdateSource = 'dfhis'
