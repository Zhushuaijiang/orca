...
export abstract class UpdaterState {
  ...
  // Why: DFHIS builds resolve the feed from package.json extraMetadata/env; 'disabled' routes all checks to the DFHIS download server instead of GitHub.
  protected releaseFeedConfig: ReleaseFeedConfig = resolveReleaseFeedConfig()
  ...
  protected isReleaseFeedDisabled(): boolean {
    return this.releaseFeedConfig.mode === 'disabled'
  }

  protected getGithubReleaseFeed(): GithubReleaseFeedConfig {
    if (this.releaseFeedConfig.mode === 'github') {
      return this.releaseFeedConfig
    }
