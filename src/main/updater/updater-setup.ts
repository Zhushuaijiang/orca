...
import { initDfhisUpdater } from '../dfhis-updater'
...
export class UpdaterSetup extends UpdaterDownloadInstall {
  ...
  setupAutoUpdater(mainWindow: BrowserWindow, opts?: UpdaterSetupOptions): void {
    this.releaseFeedConfig = resolveReleaseFeedConfig(getPackageJsonPath())
    this.mainWindowRef = mainWindow
    initDfhisUpdater((status) => this.sendStatus(status))
    ...
    const { releaseFeedConfig } = this
    if (this.activeUpdateSource === 'release' && releaseFeedConfig.mode === 'github') {
    ...
    if (this.isReleaseFeedDisabled()) {
      // Why: DFHIS builds poll their own download server instead of the GitHub feed; runBackgroundUpdateCheck routes disabled-feed checks there.
