...
import {
  ...
  markManualYunxiaoRequirementGateForWorktree
...
export class OrcaRuntimeWithControllerKnowsPtyIsLive extends OrcaRuntimeWithResolveTerminalPane {
  ...
  async sendTerminal(
    ...
    if (pty) {
      ...
      const effectiveAction = await applyYunxiaoRequirementGateForRawTerminalSend(
        { store: this.store, isTerminalRunningAgent: (h) => this.isTerminalRunningAgent(h) },
        handle,
        pty.pty.worktreeId,
        action
      )
      const payload = buildTerminalSendPayload(effectiveAction)
      if (payload === null) {
