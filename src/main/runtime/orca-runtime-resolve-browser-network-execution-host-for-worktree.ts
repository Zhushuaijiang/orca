...
import { buildYunxiaoTerminalEnv } from '../dfhis-environment/terminal-env'
...
export class OrcaRuntimeWithResolveBrowserNetworkExecutionHostForWorktree extends OrcaRuntimeWithTransitionGraphReloadToTerminalState {
  ...
  protected buildTerminalWorkspaceEnv(
    ...
    // Why: a linked Yunxiao requirement workspace carries the DFHIS environment
    // (requirement dir, code root, MCP credentials) into every spawned PTY.
    const linkedRepo = scope.folderWorkspace.linkedTask?.repoId
      ? this.store
          ?.getRepos()
          .find((repo) => repo.id === scope.folderWorkspace?.linkedTask?.repoId) ?? null
      : null
