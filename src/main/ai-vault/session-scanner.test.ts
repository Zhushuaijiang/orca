...
describe('scanAiVaultSessions', () => {
  ...
  it('indexes Codex sessions from Orca runtime homes with resumable commands', async () => {
    ...
    await mkdir(join(runtimeSessionsDir, '2026', '06', '04'), {
      recursive: true
    })
  ...
  it('indexes WSL home session roots', async () => {
    ...
    await mkdir(join(wslHome, '.claude', 'projects', 'repo'), {
      recursive: true
    })
    await mkdir(
  ...
  it('skips hidden Codex context blocks when choosing session titles', async () => {
    ...
    tempRoots.push(root)
    const roots = isolatedScanRoots(root)
    await mkdir(join(roots.codexSessionsDir, '2026', '06', '11'), {
      recursive: true
    })
  ...
  it('indexes every supported agent transcript format with native resume commands', async () => {
    ...
    const result = await scanAiVaultSessions({
      ...roots,
      platform: 'darwin',
      // Why 25: the vault holds one session per agent (21 agents), past the
      // 20-session default cap, which would drop the oldest (CodeBuddy) row.
      limit: 25
    })
