...
function devinStepTokenUsage(
  ...
  const output = firstDevinMetricValue(sources, ['output_tokens', 'completion_tokens'])
  const cacheRead = firstDevinMetricValue(sources, ['cache_read_tokens', 'cache_read_input_tokens'])
  const cacheWrite = firstDevinMetricValue(sources, [
...
export function consumeDevinSessionStep(accumulator: SessionAccumulator, step: unknown): void {
  ...
  // Key usage by the step's own model, not the accumulator fallback: the
  // streamed path applies top-level model fields only after the steps, so the
  // accumulator's mid-stream model must not decide the per-model bucket.
  const stepUsage = devinStepTokenUsage(metadata, metrics, asRecord(stepRecord.metrics))
