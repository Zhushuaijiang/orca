...
import { containsYunxiaoRequirementReference } from '../../../../shared/yunxiao-requirement-prompt-gate'
import {
  applyYunxiaoRequirementGateForPtyWrite,
  withDfHisWorkflowPackRefreshForInput
} from './yunxiao-requirement-gate'
import {
...
export function createPtyWriteInput(deps: {
  ...
  const writePtyInput = (args: PtyWritePayload): boolean | Promise<boolean> => {
    if (containsYunxiaoRequirementReference(args.data)) {
      return withDfHisWorkflowPackRefreshForInput(args.data, async () =>
        writePtyInputAfterRefresh(
          await applyYunxiaoRequirementGateForPtyWrite(args, ptyOwnership, tryGetProviderForPty)
        )
      )
    }
  ...
  const writePtyInputAccepted = (args: PtyWritePayload): boolean | Promise<boolean> => {
    if (containsYunxiaoRequirementReference(args.data)) {
      return withDfHisWorkflowPackRefreshForInput(args.data, async () =>
        writePtyInputAcceptedAfterRefresh(
          await applyYunxiaoRequirementGateForPtyWrite(args, ptyOwnership, tryGetProviderForPty)
        )
      )
    }
