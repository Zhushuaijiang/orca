...
import { buildFolderWorkspacePtyEnv } from '../host-env/dfhis-workspace-env'
...
export async function assemblePtyIpcSpawnEnv(ctx: PtyIpcSpawnState): Promise<void> {
  ...
  // Why: Yunxiao folder workspaces inject requirement-scoped env on the renderer IPC path —
  // the runtime spawn preflight only covers runtime-created terminals.
  ctx.baseEnv = buildFolderWorkspacePtyEnv(
    ctx.deps.store,
    args.worktreeId,
    ctx.cwd,
    ctx.baseEnv
