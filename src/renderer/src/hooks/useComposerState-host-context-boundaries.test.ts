...
describe('useComposerState host-context boundaries', () => {
  ...
  it('uses submit-time smart metadata for both folder launch mode and startup content', () => {
    ...
    // Why: the fork port delegates draft resolution (with its promptSeed extension) to the sidebar submit module.
    const folderSubmitSource = readFileSync(
      join(__dirname, '../components/sidebar/folder-workspace-composer-submit.ts'),
      'utf8'
    )
    expect(folderSubmitSource).toContain(
      'resolveFolderWorkspaceLaunchDraft(linkedWorkItem, note, promptSeed)'
    )
