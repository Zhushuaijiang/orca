...
export function useTaskPageWorkspaceActions(model: TaskPageSearchActionsModel) {
  const { repoMap, openModal, eligibleRepos, repoSelection } = model
  ...
  const openComposerForGitLabItem = useCallback(
    (item: GitLabWorkItem): void => {
      ...
      openModal('new-workspace-composer', {
        ...
        initialRepoId:
          getTaskWorkspaceRepoForSourceRepo(item.repoId, eligibleRepos, repoSelection)?.id ??
          repoMap.get(item.repoId)?.id ??
          item.repoId,
        telemetrySource: 'sidebar'
      })
    },
