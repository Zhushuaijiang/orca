...
export function GitLabFilesTab({ item, state, reviewActions, onOpenFiles }: Props) {
  if (item.type !== 'mr') {
    return null
  }
  const {
      ...
      {loading && !details ? (
        ...
        <>
          <div className="flex justify-end">
            ...
            >
              {translate('auto.components.GitLabItemDialog.openFilesDialog', 'Open file viewer')}
            </Button>
          </div>
          ...
          <div className="space-y-2">
            {details.files.map((file) => (
              <div key={file.path} className="rounded-md border border-border/50 bg-muted/10">
                ...
                {file.diff ? <GitLabMRFileDiff diff={file.diff} /> : (
