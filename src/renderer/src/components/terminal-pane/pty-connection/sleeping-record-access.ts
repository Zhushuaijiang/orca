...
export function installSleepingRecordAccess(session: ConnectPanePtySession): void {
  ...
  session.startupDraftPrompt =
    typeof session.paneStartup?.draftPrompt === 'string' && session.paneStartup.draftPrompt.trim()
      ? applyYunxiaoRequirementPromptGate(session.paneStartup.draftPrompt)
      : null
  session.startupInitialAgentPrompt =
    typeof session.paneStartup?.initialAgentStatus?.prompt === 'string' &&
    session.paneStartup.initialAgentStatus.prompt.trim()
      ? applyYunxiaoRequirementPromptGate(session.paneStartup.initialAgentStatus.prompt)
      : null
  session.startupAgentPrompt = session.startupInitialAgentPrompt ?? session.startupDraftPrompt
