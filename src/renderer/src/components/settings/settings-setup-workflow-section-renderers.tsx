...
export function renderDfHisEnvironmentSettingsSection(
  context: SettingsRenderContext
): React.JSX.Element {
  const { navigation, view } = context
  return (
    <SettingsSection
      ...
      searchEntries={navigation.getSectionSearchEntries('dfhis-environment')}
    >
      {view.isSectionMounted('dfhis-environment') ? <DfHisEnvironmentPane /> : null}
    </SettingsSection>
  )
}
