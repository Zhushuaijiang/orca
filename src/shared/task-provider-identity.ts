...
export function isStoredTaskProviderIdentity(provider: TaskProvider, identity: unknown): boolean {
  ...
  switch (provider) {
    ...
    case 'yunxiao':
      return ['organizationId', 'projectId', 'projectName'].every((key) =>
        isNullableOptionalString(raw[key])
      )
    case 'code-merge':
      return true
...
const TASK_PROVIDER_IDENTITY_FIELDS: Record<TaskProvider, readonly string[]> = {
  ...
  yunxiao: ['organizationId', 'projectId', 'projectName'],
  'code-merge': []
}
...
export function taskProviderIdentityCachePart(
  ...
  switch (identity.provider) {
    ...
    case 'yunxiao':
      return [identity.organizationId, identity.projectId ?? identity.projectName]
        .filter(Boolean)
        .join('/')
  }
}
